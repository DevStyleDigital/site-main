...
import {
  CardImageDropzone,
  type DropzoneFields,
} from '@/components/app/admin/card-dropzone';
...
export const PostsForm = ({ id, posts }: { id: string; posts?: iPosts }) => {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [banner, setBanner] = useState<DropzoneFields[] | undefined>(
    posts?.banner
      ? [posts?.banner].map(({ url, ...item }) => ({ file: url, ...item }))
      : undefined,
  );
  const [autorPhoto, setAutorPhoto] = useState<DropzoneFields[] | undefined>(
    posts?.bannerAutor
      ? [posts?.bannerAutor].map(({ url, ...item }) => ({ file: url, ...item }))
      : undefined,
  );
  ...
  return (
    ...
    >
      <div className="w-full flex justify-between h-full max-md:flex-col gap-10">
        <div className="w-[40%] flex flex-col gap-5 max-md:w-full">
          <div className="flex w-full flex-col gap-2">
            <Label htmlFor="title" className="font-bold text-base max-sm:text-sm">
              Título
            </Label>
            <input
              type="text"
          ...
          <div className="flex flex-col gap-2 w-full">
            <Label htmlFor="autor" className="font-bold text-base max-sm:text-sm">
              Autor
            </Label>
            <input
              type="text"
              required
              defaultValue={posts?.autor}
              name="autor"
              disabled={loading}
              id="autor"
              placeholder="Autor"
          ...
          <div className="flex flex-col gap-2 w-full">
            <Label htmlFor="tipo" className="font-bold text-base max-sm:text-sm">
              Tipo
            </Label>
            <input
              type="text"
      ...
      <button
        className="text-lg rounded-md w-fit px-3 py-2 flex gap-2 text-white bg-[#141624]"
        disabled={loading}
      >
        {posts ? (
          <>

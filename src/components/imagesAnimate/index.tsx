...
const ImagesAnimate = ({...props}: {src: any, alt: string, className: string, width: number, height: number, min:number, max:number}) => {
    ...
    return (
        ...
        >
            <motion.div variants={variants} className="w-full max-lg:flex max-lg:justify-center" style={{rotateX:a, rotateY:b}}>
                <Image ref={cardRef} src={props.src} alt={props.alt} className={props.className} width={props.width} height={props.height}  />
            </motion.div>
        </motion.div>
    )
}

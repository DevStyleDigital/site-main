    ...
    return (
        ...
        >
            {ResultsTagArray.map((result) => {
                ...
                return (
                    <motion.div variants={variants} className="flex gap-[.5rem] items-center leading-4">
                        <h1 className="text-white font-extrabold text-[2.5rem] flex items-center max-sm:text-[1.7rem] ">
                            {result.number}<span className="font-bold ml-1 text-[2rem] max-sm:text-[1.5rem]" style={{color: result.color}}>+</span>
                        </h1>
                        {result.text}
                    </motion.div>
            )})}

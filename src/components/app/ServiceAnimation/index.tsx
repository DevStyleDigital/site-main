                ...
                {ServicesUtils.map(item => {
                    ...
                    return(
                        <motion.div variants={variants}>
                            <Card.Root custom={item.id} key={item.id} className="w-full h-[500px] max-sm:h-[400px] flex flex-col justify-between gap-[2rem] p-[2rem] rounded-3xl card-color-gradient-service border-[1px] border-[#424245]">
                                <Card.Image className="w-[8rem] h-[8rem] max-sm:w-[6rem]  max-sm:h-[6rem] rounded-full flex justify-center items-center bg-[#292930] shadow-md">
                                    <Image src={item.image} className={`iconService-${item.id}`} alt={item.label} width={173} height={138} ></Image>
                                </Card.Image>

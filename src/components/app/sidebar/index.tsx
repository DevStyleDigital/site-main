...
export const SideBarFundo = () => {
  ...
  return (
    ...
    >
      <div className="w-full h-screen absolute bg-black/70" onClick={() => setSidebarOpen(false)}></div>
        <div className='relative z-40 float-right w-[60%] h-full bg-black/95'>
            <div className="absolute w-full flex flex-col gap-12 p-[2rem] top-0 h-full z-50">
                <div className='w-full flex justify-end'>
                    <button className="w-[3rem] h-[3rem] bg-white rounded-full shadow-md flex justify-center items-center" onClick={() => setSidebarOpen(false)}>
                    ...
                    <ul className="flex-col items-center gap-[2rem] hidden max-md:flex">
                        {NavsItems.map(item => {
                            return (
                                <li key={item.id} className={clsx('font-medium text-white', {

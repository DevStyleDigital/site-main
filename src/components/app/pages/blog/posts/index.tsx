        ...
        {AllPosts.map((blogPost: any) => {
          return (
            <Link
              ...
              target="_blank"
              key={blogPost.id}
              className="w-full min-h-[460px] max-sm:min-h-0 max-sm:h-fit bg-white h-full border border-[#E8E8EA] p-4 rounded-xl flex flex-col gap-5"
            >
              ...
              <div className="w-full h-full justify-between flex flex-col px-2 gap-4 max-sm:gap-8">
                <div className="flex flex-col gap-2">
                  <span className="w-fit px-2 py-1 bg-[#4B6BFB]/5 rounded-sm text-[#4B6BFB] text-sm">
                    {blogPost.type}
                  </span>
                  <h2 className="font-bold text-xl max-w-[70%] break-words text-secondary_black-950 max-[1600px]:max-w-[90%]">
                    {blogPost.title}
                  </h2>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex gap-2 items-center">
                    ...
                    <span className="font-medium text-sm text-black/60 max-sm:text-xs">
                      {blogPost.autor}
                    </span>
                  </div>
                  <span className="font-medium text-sm text-black/60 ml-10 max-sm:text-xs">
                    {formateDate(blogPost.created_at)}
                  </span>
                </div>
              </div>
            </Link>

...
const Hero = ({children, className}: {children: ReactNode, className: string}) => {
    return (
        <div className={`${className} w-full h-[90vh] flex flex-col gap-[.7rem] items-center justify-center px-[4rem] relative overflow-hidden max-lg:text-center `}>
            {children}
        </div>
    )
}
...
export const RotaHero = ({children} : {children: ReactNode}) => {
    return (
        <motion.span variants={item} className="font-bold text-[.8rem] max-lg:text-[1rem] max-sm:text-[.8rem]">{children}</motion.span>
    )
}

export const SubHero = ({children} : {children: ReactNode}) => {
    return (
        <motion.span variants={item} className="font-normal z-10 text-[1.1rem] max-w-[38rem] tracking-wider mb-[1.5rem] text-neutral-500 max-xl:text-[.8rem] max-xl:max-w-[25rem] max-lg:max-w-[30rem] max-lg:text-[1rem] max-lg:text-neutral-700 max-sm:text-[.8rem]">{children}</motion.span>
    )
}

  ...
  return(
    <Popover>
      ...
      <PopoverContent className="w-fit rounded-sm bg-white" align="start">
        <button type="button" className="flex items-center gap-2 text-sm" onClick={signOut}><LogoutOutlinedIcon className="w-5 h-5" /> Logout</button>
      </PopoverContent>    
    </Popover>
  )
}
              ...
              <li key={links.id}>
                  <Link
                    className={clsx(
                      ...
                      {"bg-black/10 rounded-sm font-semibold": pathname == links.href || pathname == links?.subLink}
                    )}
                    href={links.href}
                  >
                    <span className={clsx("w-0 h-5 bg-red-700 rounded-lg absolute left-0 transition-all top-0 translate-y-[50%]", {'!w-1': pathname == links.href  || pathname == links?.subLink})} />
                    {links.icon}
                    {links.label}
                  </Link>
              </li>
